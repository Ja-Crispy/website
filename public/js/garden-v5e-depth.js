...
const PlantField = {
  ...
  build(data, w, h) {
    ...
    // depth layers: front ground at h-50, back ground at h-200
    const frontY = h - 50;
    const backY = h - 200;

    // scatter positions using golden angle in 2D (x + depth)
    const padX = 40;
    ...
    sorted.forEach((item, i) => {
      ...
      // depth: 0 = front, 1 = back (use golden angle Y component)
      const depth = Math.abs(Math.sin(theta * 1.3 + rng() * 0.5)) * 0.85 + rng() * 0.15;
      const groundY = frontY - depth * (frontY - backY);
      const depthScale = 1 - depth * 0.4;  // back trees 60% size
      const depthDim = 1 - depth * 0.35;   // back trees 65% brightness
    ...
    // sort back-to-front so foreground renders on top
    this.plants.sort((a, b) => b.depth - a.depth);
...
const GardenRenderer = {
  ...
  draw() {
    ...
    // --- starfield nebula background (falling drift) ---
    ...
    // --- atmospheric fog (top fades into void) ---
    const fogGrd = ctx.createLinearGradient(0, 0, 0, h * 0.55);
    fogGrd.addColorStop(0, 'rgba(10,10,12,0.3)');
    fogGrd.addColorStop(1, 'rgba(10,10,12,0)');
  ...
  _drawTree(ctx, plant, gp, dimmed, breath, t) {
    const depthAlpha = plant.depthDim || 1;
    const alpha = dimmed ? 0.06 : depthAlpha;
